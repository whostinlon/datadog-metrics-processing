...
func queryMetric(ctx *context.Context, numberOfDaysBack int, apiClient *datadog.APIClient, category string, offerID string) (datadog.MetricsQueryResponse, *http.Response, error) {
	if category == "success" || category == "failure" {
		response, r, err := apiClient.MetricsApi.QueryMetrics(*ctx, time.Now().AddDate(0, 0, -numberOfDaysBack).Unix(), time.Now().Unix(),
			`sum:prometheus.payflow_subscription_`+category+`_total{env:prd, offer_id:`+offerID+`} by {offer_id, product_type}.as_count()`)
		if err != nil {
			return datadog.MetricsQueryResponse{}, r, fmt.Errorf("failed to get data from payflowV2: %v: %w", err, r)
		}
		return response, r, err
...
func fetchData(numberOfDaysBack int, offerID string) (map[string]interface{}, error) {
	if os.Getenv("DD_API_KEY") == "" || os.Getenv("DD_APP_KEY") == "" { // Datadog API Client requirement
		return nil, fmt.Errorf("failed to fetch environment variables")
	}
	...
	successResp, responsePointer, err := queryMetric(&ctx, numberOfDaysBack, apiClient, "success", offerID)
	if err != nil {
		return nil, fmt.Errorf("failed to get success purchase data: %v: %w", responsePointer, err)
	}

	failureResp, responsePointer, err := queryMetric(&ctx, numberOfDaysBack, apiClient, "failure", offerID)
	if err != nil {
		return nil, fmt.Errorf("failed to get failed purchase data: %v: %w", responsePointer, err)
	}
	...
	for _, series := range successData.Series {
		if series.TagSet[tagSetOfferID] != "offer_id:"+offerID {
			continue
		}
	...
	return nil, fmt.Errorf("offer id not found")
}
